...
class CorpusMetrics(BaseStatistics):
    def __init__(self, word_count, vocab_analyzer):
        self.word_counter = word_count
        self.vocab_analyzer = vocab_analyzer

    def calculate(self, df):
        word_counts = self.word_counter.calculate_word_count(df, "clean_text")
        total_words = self.word_counter.calculate_total_words(word_counts)

        word_counts_no_stop = self.word_counter.calculate_word_count(df, "no_stopwords")
        total_no_stopwords = self.word_counter.calculate_total_words(
            word_counts_no_stop
        )

        base_stats = self.word_counter.calculate_statistics(word_counts, total_words)
